...
class graph {
  ...
  public:
    ...
    void print_weight_sum_two_connected_components() {
      ll sum = 0;
      for(auto x: t) {
        sum += x.first;
      }
      // In the minimum spanning tree, delete the longest edge to make a two
      // fully connected trees.
      cout << sum - t[n-2].first << '\n';
    }
};
...
int main () {
  ...
  g.read_edges();
  g.kruskal();
  g.print_weight_sum_two_connected_components();
  return 0;
}
