...
public:
  ...
  // 2. thus, only if row flipping makes the identical patterns for every row,
  // it can be 0.
  bool removeOnes(vector<vector<int>> &grid) {
    ...
    for (int i = 1; i < m; i++) {
      ...
      for (int j = 0; j < n; j++) {
        if (ref_row[j] != grid[i][j])
          identical = false;
        if (ref_row_inverted[j] != grid[i][j])
          invert_identical = false;
      }
      if (!identical && !invert_identical)
        return false;
    }
    return true;
  }
