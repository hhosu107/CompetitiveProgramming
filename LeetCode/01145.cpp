...
public:
  int countNodesWithRoot(TreeNode *root) {
    if (root == nullptr)
      ...
      return 1 + countNodesWithRoot(root->left) +
             countNodesWithRoot(root->right);
  }
  bool btreeGameWinningMove(TreeNode *root, int n, int x) {
    ...
    // If x == root's value? -> check left/right child's count. If left is more,
    // choose left / vice versa. Finally, check whether or not the count
    // exceeds. If x == one of the childs? -> take the direct parent. And count
    // number of the nodes that under x.
  }
};
*
