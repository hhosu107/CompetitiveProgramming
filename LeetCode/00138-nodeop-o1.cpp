...
public:
  Node *copyRandomList(Node *head) {
    if (head == nullptr)
      return nullptr;
    // From the linked list A->B->C->..., make a linked list
    // A->A'->B->B'->C->C'->... (original's address value remains the same)
    Node *ptr = head;
    while (ptr != nullptr) {
    ...
    // if original node's random pointer is ptr->random, by construction, the
    // copy pointer's random must be ptr->random->next!
    while (ptr != nullptr) {
      ptr->next->random =
    ...
    while (ptr_old_list != nullptr) {
      ptr_old_list->next =
          ptr_old_list->next->next; // recover A->B from A->A'->B
      ptr_new_list->next = (ptr_new_list->next != nullptr)
                               ? ptr_new_list->next->next
                               : nullptr; // recover A'->B' from A'->B->B'
