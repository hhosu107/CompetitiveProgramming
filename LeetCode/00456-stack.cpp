...
public:
  bool find132pattern(vector<int> &nums) {
    ...
    for (int j = nums.size() - 1; j >= 0; j--) {
      ...
      // Now the first element of the stack must be bigger than mins[j]. Thus,
      // compare that element with nums[j].
      if (!right_numbers.empty() && right_numbers.top() < nums[j]) {
        return true; // then, right_numbers.top() > mins[j],
                     // right_numbers.top().idx > j > mins[j].idx,
                     // right_numbers.top() < nums[j]. PROFIT
      }
