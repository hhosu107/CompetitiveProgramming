...
public:
  bool canConvert(string str1, string str2) {
    // 1. Initial state에서 같은 character가 target의 서로 다른 character로
    // mapping되어야 하는 상황이 있다면 false.
    // 2. Initial state에서 어떤 character가 target의 character로 mapping될 때,
    // target의 character가 이미 initial state에 나타나있는 상황이라면 chain.
    // chain의 마지막에, initial state에는 없는 character여야 한다.
    // 만약 chain이 loop를 만들고 있다면, swapping rule에 의해, 어떤 character를
    // temporary state로 둬야 하는데, 이 때 이 character는 initial state 및
    // target 어디에도 없는 character여야 한다. 그런 character가 존재하지
    // 않는다면 false.
    // 1번 감지는 쉽고, 2번의 경우, cycle detection을 통해, cycle이 있다면 key +
    // value 통틀어 최대 25개의 english character만 존재해야 하며 (단 self cycle
    // 26개인 경우 예외), 아니면 26개 다 있어도 된다.
    ...
    for (auto it = mapped_count.begin(); it != mapped_count.end(); it++) {
      if (it->second == 0 &&
          init_to_target_excluding_self_cycle.find(it->first) !=
              init_to_target_excluding_self_cycle.end())
    ...
    // return true;
    return (used_pool_size <= 25) || non_cycle_chars > 0 ||
           (self_cycle_cnt == used_chars_in_target.size());
    // each condition matches: at most 25 different characters in the str1 +
    // str2, at most 25 different characters in the str2, identicality
  }
};
