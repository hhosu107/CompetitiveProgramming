...
public:
  ...
  int romanToInt(string s) {
    ...
    vector<string> finding_order{
        "MMM", "MM", "M",    "CM",   "DCCC", "DCC", "DC", "D",   "CD",  "CCC",
        "CC",  "C",  "XC",   "LXXX", "LXX",  "LX",  "L",  "XL",  "XXX", "XX",
    ...
    while (parsed_chars < s_len && seek_patterns < patterns_len) {
      if (finding_order[seek_patterns].length() + parsed_chars <= s_len &&
          s.substr(parsed_chars, finding_order[seek_patterns].length()) ==
              finding_order[seek_patterns]) {
