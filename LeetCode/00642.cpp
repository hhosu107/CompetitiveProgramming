...
public:
  ...
  vector<pis> prefix_search(string prefix) {
    ...
    while (!node_prefix.empty()) {
      ...
      for (auto it = ((hd.first)->children).begin();
           it != ((hd.first)->children).end(); it++)
 ...
 * Reference:
 * https://leetcode.com/problems/design-search-autocomplete-system/discuss/105393/C%2B%2B-Trie-and-DFS-solution-beats-80
 * */
class AutocompleteSystem {
public:
  // Sol) Trie, but the ending-indicator is not a boolean but an integer that
  // were typed.
  string curr_stream;
  TrieNode *root;
