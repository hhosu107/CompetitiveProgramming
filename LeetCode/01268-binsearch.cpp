...
public:
  vector<vector<string>> suggestedProducts(vector<string> &vp, string w) {
    ...
    for (const auto ch : w) {
      ...
      // buf.
      // To optimize this once more, save cit1(it_start) and cit2(it_end) for
      // each iteration and reuse to find lower_bound between them.
      ...
      // 가능하다.
      // 다만 static_assert(sizeof(int) == 4)를 해서 주소값(iterator)이
      // 4바이트인 것을 확실하게 해야 함. emplace_back이 push_back보다 성능 상
      // 더 유리하다. emplace_back은 삽입할 객체의 생성자를 위한 인자들을 받아
      // vector<> 내에서 직접 객체를 생성하여 삽입한다. 위의 경우
      // vector<string>을 생성할 수 있는 인자만 받으면 되는데, vector<>를 생성할
      // 때 특정 container의 begin, end const iterator를 받으면 바로 되기 때문에
      // 저건 valid한 코드이다. push_back은 삽입할 객체를 직접 만들어서 넣어야
      // 하므로 성능상 불리하다. ex) 만약 vector<int>에 emplace_back하는 거라면
      // push_back과 완전히 같을 것.
    }
