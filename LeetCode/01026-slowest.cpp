...
public:
    ...
    set<int> maxAncestorDiffSet(TreeNode* root) {
        if (root == nullptr) return set<int>();
        if (root->left == nullptr && root->right == nullptr) return set<int>{root->val};
...
// compute maximum difference between root and left nodes / root and right nodes,
// and return the set of values in this subtree.
