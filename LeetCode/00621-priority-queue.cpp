struct TaskLeftAble {
    ...
    TaskLeftAble(char t, int l, int a) : task(t), left_tasks(l), able_timestamp(a) {}
    
    bool operator<(const TaskLeftAble &o) const {
        if (left_tasks != o.left_tasks) return left_tasks < o.left_tasks;
        if (able_timestamp != o.able_timestamp) return able_timestamp > o.able_timestamp;
        return (task > o.task);
    }
    ...
    int leastInterval(vector<char>& tasks, int n) {
        ...
        while(!pq.empty()) {
            ...
            if (curr.left_tasks > 1) {
                waiting.push(TaskLeftAble(curr.task, curr.left_tasks - 1, unit_time + n + 1));
            }
            if (!waiting.empty() && (pq.empty() || unit_time + 1 >= waiting.front().able_timestamp)) {
                do {
                    TaskLeftAble next = waiting.front();
                    waiting.pop();
                    pq.push(next);
                } while (!waiting.empty() && unit_time + 1 >= waiting.front().able_timestamp);
            }
        }
        return unit_time;
    }
