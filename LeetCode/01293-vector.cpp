...
public:
  ...
  PointViolation(int _s, int _x, int _y, int _v)
      : steps(_s), x(_x), y(_y), v(_v) {}
...
struct MyHash {
  std::size_t operator()(PointViolation const &s) const noexcept {
    ...
    /*
            ...
            std::size_t h12 = h1 ^ (h2 << 1); // or use boost::hash_combine (see
    Discussion) std::size_t h3 = std::hash<std::int32_t>{}(s.steps); std::size_t
    h4 = std::hash<std::int32_t>{}(s.v); std::size_t h34 = h3 ^ (h4 << 1); // or
    use boost::hash_combine (see Discussion) return h12 ^ (h34 << 1);
    */
...
public:
  int shortestPath(vector<vector<int>> &grid, int k) {
    ...
    if (k >= m + n - 2)
      return m + n - 2; // just go down and go right.
    vector<vector<vector<int>>> dist(
        m, vector<vector<int>>(n, vector<int>(k + 1, k * (m * n + 1))));
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        PointViolation npv{dist[curr.x][curr.y][curr.v] + 1, nx, ny,
                           next_violation};
        if (next_violation >= 0 &&
            dist[nx][ny][next_violation] == k * (m * n + 1)) {
          if (nx == m - 1 && ny == n - 1) {
            return npv.steps;
          }
