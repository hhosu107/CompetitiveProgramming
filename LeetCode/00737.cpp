...
public:
    bool areSentencesSimilarTwo(vector<string>& sentence1, vector<string>& sentence2, vector<vector<string>>& similarPairs) {
        ...
        for(int i=0; i<sentence1.size(); i++) {
            ...
            if (similarPairMap.find(sentence1[i]) == similarPairMap.end() || similarPairMap.find(sentence2[i]) == similarPairMap.end()) {
                return false;
            }
            queue<string> trans;
            ...
            while(!trans.empty()) {
                ...
                trans.pop();
                for(auto it=similarPairMap[curr].begin(); it != similarPairMap[curr].end(); it++) {
