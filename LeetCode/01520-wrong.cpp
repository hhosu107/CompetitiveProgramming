    ...
    */
    struct charOccurrenceEvent {
        ...
        bool operator<(const charOccurrenceEvent &o) {
            return timestamp < o.timestamp || (timestamp == o.timestamp && first_occur_last_occur == false && o.first_occur_last_occur == true);
        }
    };
    struct charChunk {
        int x;
        ...
        bool included(const charChunk &o) {
            return x > o.x && y < o.y;
        }
    };
    vector<string> maxNumOfSubstrings(string s) {
        // Sol) Perform the union find on the segment. One exception: if one includes the other one, remove that encapsules one instead of merging.
        ...
        map<set<char>, pair<int, int>> char_chunk_timestamp; // pair's first: first occurrence of the set; pair's last: last occurrence of the set.
        /*
        ...
        */
        ...
        for(charOccurrenceEvent e: events) {
            if (e.first_occur_last_occur) {
                ...
                char_chunk_timestamp.insert({curr_chars, make_pair(e.timestamp, e.timestamp)});
            } else {
                set<char> prev_chars = curr_chars;
                curr_chars.erase(e.c);
                if (char_chunk_timestamp.find(curr_chars) == char_chunk_timestamp.end()) {
                    char_chunk_timestamp.insert({curr_chars, make_pair(e.timestamp, e.timestamp)});
                } else {
