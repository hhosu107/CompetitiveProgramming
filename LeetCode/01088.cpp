...
public:
    int confusingNumberII(int n) {
        /* Sol) For 5^k digit strings, compute number of non palindromic confusing numbers. */
        /* For each pair of endpoints, there will be non-confusing pairs: (0, 0), (1, 1), (8, 8), (6, 9), (9, 6) (heading three are valid when inside, tailing two are valid when not in center)
        len 1 invalid confusion numbers: 3 (0, 1, 8). len 2 invalid confusion numbers: 4 (11, 88, 69, 96). len 3 invalid confusion numbers: 4 * 3. len 4 invalid confusion numbers: 4 * 5., ...
        inner len 1 invalid confusion numbers: 3, inner len 2 invalid confusion numbers: 5. inner len 3 invalind confusion numbers: 5 * 3. inner len 4 invalid confusion numbers: 5 * 5, ...
        len x invalid confusion numbers: len x-2 inner invalid confusion numbers * 4
        */
        // 1-digit exceptions
        if (n < 6) return 0;
        if (n < 9) return 1;
        ...
        for(int i=0; i<num_digits; i++) {
            if (i > 0 && !confusing_until[i - 1]) break;
            for(int j=0; j<n_digits[i]; j++) {
                if (i == 0 && j == 0) {
                    for(int i=0; i<=num_digits - 2; i++) {
                        cout << count_confusing_numbers[i] << " " << count_invalid_numbers[i] << endl;
                        valid_confusion_numbers += (count_confusing_numbers[i] - count_invalid_numbers[i]);
                    }
                }
                else if (confusing_digit.find(j) != confusing_digit.end()){
                    // Fix ith highest digit as j, Fix 0-i-1th highest digit as n_digits[k].
                    // Now Fill their corresponding confusing digits from the back.
                    // In this condition, compute the number of invalid confusing numbers/
                    int num_confusing_numbers = (i == num_digits - 1 ? 1 : count_full_confusion_numbers[num_digits - 2 - i]);
                    ...
                    for(int k=0; k<i; k++) {
                        ...
                        if (k >= num_digits - 1 - k) {
                            if (fixed_digits[num_digits - 1 - k] != confusing_pair[n_digits[k]]) {
                    ...
                    if (fixed_invalid_confusion_number) {
                        if (2 * (i + 1) >= num_digits)
                            ...
                            found_invalid_confusion_numbers = count_invalid_numbers_inner[num_digits - 2 * (i + 1) - 1];
                    }
        ...
        if (!confusing_until[num_digits - 1]) last_number_confusing = 0;
        else {
            ...
            if (invalid_confusing) last_number_confusing = 0;
        }
