...
public:
    string addBoldTag(string s, vector<string>& words) {
        ...
        for(int i=0; i<found_indices_rev.size() - 1; i++) {
            result += "<b>";
            result += s.substr(found_indices_rev[i].first, found_indices_rev[i].second - found_indices_rev[i].first);
            result += "</b>";
            result += s.substr(found_indices_rev[i].second, found_indices_rev[i+1].first - found_indices_rev[i].second);
        }
        result += "<b>";
        result += s.substr(found_indices_rev[found_indices_rev.size() - 1].first, found_indices_rev[found_indices_rev.size() - 1].second - found_indices_rev[found_indices_rev.size() - 1].first);
        result += "</b>";
