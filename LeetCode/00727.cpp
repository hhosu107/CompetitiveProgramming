...
public:
    string minWindow(string s1, string s2) {
        ...
        // DP[s2_index][s1_index]: smallest index in x whose character equals to s2[s2_index].
        // How to fill? loop from s2.len - 1 to 0; loop from s1.len - 1 to 0.
        ...
        for(int i=0; i<=s1_len - s2_len; i++) {
            int next_idx = dp[0][i];
            if (tried_starts.find(next_idx) != tried_starts.end()) continue;
