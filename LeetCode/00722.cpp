    ...
    vector<string> removeComments(vector<string>& source) {
        ...
        for(int i=0; i<source_lines; i++) {
            ...
            while (j < source[i].length()) {
                if (source[i][j] == '*') {
                    if (encountered_block_comment && j + 1 < source[i].length() && source[i][j + 1] == '/') {
                        encountered_block_comment = false;
                        j += 2;
                    } else if (encountered_block_comment) j += 1;
                    else {
                    ...
                    else if (!encountered_block_comment && j + 1 < source[i].length() && source[i][j + 1] == '*') {
