...
public:
  vector<int> shortestDistanceColor(vector<int> &colors,
    ...
    for (int i = 0; i < queries.size(); i++) {
      ...
      auto left_idx = lower_bound(colors_idx[color_idx].begin(),
                                  colors_idx[color_idx].end(), queries[i][0]);
      auto right_idx = upper_bound(colors_idx[color_idx].begin(),
                                   colors_idx[color_idx].end(), queries[i][0]);
      int left_value;
      int right_value;
      if (right_idx == colors_idx[color_idx].end()) {
        ...
        left_value = right_value =
            colors_idx[color_idx][right_idx - colors_idx[color_idx].begin()];
        ...
        left_value =
            colors_idx[color_idx][left_idx - colors_idx[color_idx].begin()];
        right_value =
            colors_idx[color_idx][right_idx - colors_idx[color_idx].begin()];
      }
