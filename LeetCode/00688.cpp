...
public:
  unordered_map<string, double>
      move_prob; // key = [row, col, how_many_moves_left], prob = probability
                 // that alives on the board after movements
  ...
  double find(int n, int moves, int r, int c) {
    ...
    if (move_prob.find(mem_key) != move_prob.end())
      return move_prob[mem_key];
    double sum_prob = 0.0;
    for (int i = 0; i < 8; i++) {
