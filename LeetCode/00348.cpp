...
public:
  ...
  int move(int row, int col, int player) {
    ...
    if (row == col)
      diag_count[player - 1][0] += 1;
    if (row + col == size - 1)
      diag_count[player - 1][1] += 1;
    if (row_count[player - 1][row] == size ||
        col_count[player - 1][col] == size ||
        diag_count[player - 1][0] == size || diag_count[player - 1][1] == size)
      return player;
    return 0;
  }
};
