...
public:
    ...
    static bool case_insensitive_equality(string pattern, string query) {
        transform(pattern.begin(), pattern.end(), pattern.begin(), [](unsigned char c) { return tolower(c);});
        transform(query.begin(), query.end(), query.begin(), [](unsigned char c) { return tolower(c);});
        return pattern == query;
    }

    static bool wildcard_equality(string pattern, string query) {
        transform(pattern.begin(), pattern.end(), pattern.begin(), [](unsigned char c) { char lc = tolower(c); set<char> vowels{'a', 'e', 'i', 'o', 'u'}; return vowels.find(lc) != vowels.end() ? '-' : lc;});
        transform(query.begin(), query.end(), query.begin(), [](unsigned char c) { char lc = tolower(c); set<char> vowels{'a', 'e', 'i', 'o', 'u'}; return vowels.find(lc) != vowels.end() ? '-' : lc;});
        return pattern == query;
    }
    ...
    string query_exists(string word) {
        ...
        if (root->matched_string_size == 0) return "";
        // exact equality
        ...
        for(string m: root->matched_strings) {
            if (case_insensitive_equality(m, word)) return m;
        }
        // wildcard_equality
        for(string m: root->matched_strings) {
            if (wildcard_equality(m, word)) return m;
        }
        return "";
    }
};
...
public:
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) {
        ...
        // When a character is one of upper/lower vowels, that character is matched with a special character '-'.
