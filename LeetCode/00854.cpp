...
public:
    ...
    long long int converted_hash(int len, int p1, int p2, long long int source_hash) {
    ...
    bool convertible(int len, int p1, int p2, long long int source_hash, long long int target_hash) {
        long long int source_p1 = position_hash(len, p1, source_hash);
        long long int source_p2 = position_hash(len, p2, source_hash);
        long long int target_p1 = position_hash(len, p1, target_hash);
        long long int target_p2 = position_hash(len, p2, target_hash);
        if (source_p1 == source_p2 || target_p1 == target_p2) return false;
        if (source_p1 == target_p1 && source_p2 == target_p2) return false;
        if (source_p1 == target_p2 || source_p2 == target_p1) return true;
        return false;
    }
    /*
    int kSimilarity(string s1, string s2) {
        // Sol) Use map for hashset. States: max 8^20, but much more under.
        // Edge 가지치기: current string, target string이 있을 때, current string의 두 포지션을 바꿨을 때 어느 한 쪽이 target string의 한 쪽과 일치애햐만 convert 가능.
        if (s1 == s2) return 0;
        using ll = long long;
        using pli = pair<ll, int>;
        int len = s1.length();
        long long int s1_hash = string_hash(s1);
        ...
        while(!hash_dist.empty()) {
            ...
            for (int i=0; i<len - 1; i++) {
                for(int j=i+1; j<len; j++) {
                    bool is_convertible = convertible(len, i, j, next_pair.first, s2_hash);
                    if (!is_convertible) continue;
                    ll next_hash = converted_hash(len, i, j, next_pair.first);
                    if (next_hash == s2_hash) return next_pair.second + 1;
                    if (visited_hashes.find(next_hash) != visited_hashes.end()) continue;
    ...
    int kSimilarity(string A, string B)
    {
        ...
        for (int j = i + 1; j < A.size(); j++)
        {
            ...
            if (A[j] != B[j] && A[j] == B[i])
            {
                ...
                // because we match two chars in a single swap, it is the max benefit we could get from a single swap)
                // it must be the optimal solution, we just need to return the res and skip the rest of the code
                // and don't need to swap back since we won't trace back. 
               
                // I think it is the most genius part of the code, which helps to improve the performance significantly.
                // the code could also work without this part, but the runtime is 826 ms, which is much slower than 16ms
...
// 다만, 만약 어떤 j(최소값)에 대해 x[i] == y[j]마저 성립한다면, 이 둘을 바꿔버리면 미래의 어떤 index j까지
// 즉시 고정 가능하므로 위에서 취하던 후보들을 볼 필요조차 없이 이게
// optimal하다. 따라서 둘을 교환한 다음 x[i+1:]과 y[i+1:]만 보도록 fast
// return해버리면 된다.
