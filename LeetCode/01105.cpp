...
public:
    int minHeightShelves(vector<vector<int>>& books, int shelfWidth) {
        ...
        for(int i = 0; i < n; i++)
        {
            dp[i + 1] = dp[i] + books[i][1]; // maximum value: i'th book is placed at the new layer.
            int sum = 0, h = 0;
            for(int j = i; j >= 0; j--) // Sweep previous books in the reverse order
            {
                ...
                {
                    h = max(h, books[j][1]); // Since only the new book we watch affects the maximum height, O(1)
                    dp[i + 1] = min(dp[j] + h, dp[i + 1]); // We pull out j'th book to the new layer. So we have sum of heights at j-1th book and the height h. Compare dp[j] + h and dp[i + 1] and update dp[i + 1].
                }
            }
        }
        return dp[n];
