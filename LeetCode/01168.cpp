...
public:
    int minCostToSupplyWater(int n, vector<int>& wells, vector<vector<int>>& pipes) {
        ...
        // sort pipes with cost increasing order. do find for them, and if they are in the different representatives, add that pipe.
...
// Since the well is the virtual node that is already connected with water source, add 0 as the new house that already contains water.
// If every house is connected with node 0, we are done.
// construct [0, house_i, wells[i]] edge for that.
