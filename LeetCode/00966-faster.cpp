...
public:
    ...
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) {
        ...
        for(string w: wordlist) {
            ...
            transform(w.begin(), w.end(), w.begin(), [](char c){if (is_vowel(c)) { return '*';} return c;});
        ...
        for(string q: queries) {
            ...
            transform(q.begin(), q.end(), q.begin(), [](char c){return tolower(c);});
            if (words_lower.find(q) != words_lower.end()) {
                answer.push_back(words_lower[q][0]);
                continue;
            }
            transform(q.begin(), q.end(), q.begin(), [](char c){if (is_vowel(c)) { return '*';} return c;});
