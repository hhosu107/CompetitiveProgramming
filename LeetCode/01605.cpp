...
public:
    vector<vector<int>> restoreMatrix(vector<int>& rowSum, vector<int>& colSum) {
        ...
        // We need (0, 0) ~ (x2, y2), (0, 0) ~ (x1 - 1, y2), (0, 0) ~ (x2, y1 - 1), (0, 0) ~ (x1 - 1, y1 - 1).
        // How to compute (0, 0) ~ (x, y) then?
        // At least we can compute difference between (0, 0) ~ (x, y) and (x + 1, y + 1) ~ (#rows - 1, #cols - 1) in O(1).
        // (0, 0) ~ (x, y) + (x + 1, y + 1) + all = 
        /*
        [5, 0, 0]
        [1, 6, 0]
        ...
        */
        ...
        // [0, 4, 0]
        // [0, 1, 3]
        vector<vector<bool>> cell_fixed(rowSum.size(), vector<bool>(colSum.size(), false));
