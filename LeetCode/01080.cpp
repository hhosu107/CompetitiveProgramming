...
public:
  ...
  TreeNode *sufficientSubset(TreeNode *root, int limit) {
    // Sol) For each node, maintain the max path value; if it doesn't, the
    // parent will remove that path.
    int root_pathmax = pathMax(root, limit, 0);
    if (root_pathmax < limit)
      return nullptr;
    return root;
  }
};
