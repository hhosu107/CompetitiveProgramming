  ...
  vector<double> calcEquation(vector<vector<string>> &equations,
    ...
    for (int i = 0; i < ql; i++) {
      ...
      while (!curr_values.empty()) {
        ...
        for (auto it = graph[curr.second].begin();
             it != graph[curr.second].end(); it++) {
          if (visited_vars.find((*it).second) != visited_vars.end())
            continue;
    ...
    // a -> b.
    // query에 identical string: 1) query has known variables. -> a / a = 1. 2)
    // query has unknown variables. -> x / x -> -1. query에 주어진 두 variable이
    // 연결되어있지 않은 경우 -> -1.
  }
};
