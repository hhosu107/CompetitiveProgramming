...
public:
  ...
  void push(int val) {
    ...
    elmt_freq[val] += 1;
    if (elmt_level.count(elmt_freq[val]) == 0 ||
        elmt_level[elmt_freq[val]].empty()) {
      elmt_level[elmt_freq[val]] = stack<int>();
      if (max_level < elmt_freq[val])
        max_level = elmt_freq[val];
    }
    elmt_level[elmt_freq[val]].push(val);
  }

  int pop() {
    // To maintain elmt_level size in O(1) time, when the pop occurs, if that
    // level's element gets zero, delete that key from the map.
    int target_num = elmt_level[max_level].top();
    elmt_level[max_level].pop();
    if (elmt_level[max_level].size() == 0)
      max_level--;
...
//   this.inFloor[val]++;
//   if(!this.doubleStack[this.inFloor[val]])
//   this.doubleStack[this.inFloor[val]] = []
