...
struct TrieNodePos {
    ...
    bool operator<(const TrieNodePos &o) const {
        return index < o.index || (index == o.index && loc < o.loc) || (index == o.index && loc == o.loc && word_split < o.word_split);
    }
};

class Solution {
public:
    void move_queue_once(queue<TrieNodePos> &q, set<TrieNodePos> &ts, string &s, int len, TrieNode *root) {
        TrieNodePos curr = q.front();
        q.pop();
        ts.erase(curr);
        if (curr.index == len) return;
        TrieNode *next_curr = curr.loc->move_node(s[curr.index]);
        if (next_curr == nullptr) return;
        if (next_curr->is_word) {
            string appended_word = curr.word_split + (curr.word_split == "" ? "" : " ") + next_curr->word;
