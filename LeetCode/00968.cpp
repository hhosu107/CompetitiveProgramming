...
public:
    ...
    void treeCover(TreeNode *parent, TreeNode *curr) {
	    ...
	    if ((!covered[curr] && parent == nullptr) || (curr->left != nullptr && !covered[curr->left]) || (curr->right != nullptr && !covered[curr->right])) {
    		used.insert(curr);
...
parentHasCamera : 0
-> ret = min(ret, 1 + getMinCamera(root->left->left, 1) + getMin(root->left->right, 1) + getMin(root->right, 0);
-> ret = min(ret, 1 + getMinCamera(root->right->left, 1) + getMin(root->right->right, 1) + getMin(root->left, 0);
*/
