...
public:
    ...
    QuadNode* ancestor;
    QuadNode* child;
    QuadNode(int val) : val(val), prev(nullptr), next(nullptr), ancestor(nullptr), child(nullptr) {}
    QuadNode* search(int target) {
        if (this->val == target) return this;
        else if (this->next->val <= target) return this->next->search(target);
        else if (this->child == nullptr) return this;
        else return this->child->search(target);
    }
    ~QuadNode();
};
...
public:
    ...
    bool erase(int target) {
        ...
        if (found != nullptr && found->val == target) {
            while (found->ancestor != nullptr) found = found->ancestor;
            while (found != nullptr) {
...
public:
    ...
    bool search(int target) {
        return leveled_lists[19].search(target);
    }
    ...
    bool erase(int num) {
        return leveled_lists[19].erase(num);
    }
};
