    ...
    void removeNode(set<TreeNode*> &forest, int delete_val) {
        for (auto it = forest.begin(); it != forest.end(); it++) {
            if ((*it)->val == delete_val) {
                if ((*it)->left != nullptr) forest.insert((*it)->left);
                if ((*it)->right != nullptr) forest.insert((*it)->right);
                forest.erase(it);
                return;
            }
            ...
            while(!nodes.empty()) {
                ...
                if ((curr.second)->val == delete_val) {
                    if ((curr.second)->left != nullptr) forest.insert((curr.second)->left);
                    if ((curr.second)->right != nullptr) forest.insert((curr.second)->right);
                    if (((curr.first)->left) != nullptr && ((curr.first)->left)->val == delete_val) {
                        
                        (curr.first)->left = nullptr;
                    }
                    if (((curr.first)->right) != nullptr && ((curr.first)->right)->val == delete_val) {
