...
public:
    vector<int> shortestAlternatingPaths(int n, vector<vector<int>>& redEdges, vector<vector<int>>& blueEdges) {
        ...
        min_dist[0] = 0;
        for(int i=1; i<n; i++) {
            if (dist[i] != -1 && dist[i + n] != -1) min_dist[i] = min(dist[i], dist[i + n]);
            else if (dist[i] != -1) min_dist[i] = dist[i];
            else if (dist[i + n] != -1) min_dist[i] = dist[i + n];
        }
        return min_dist;
    }
};
