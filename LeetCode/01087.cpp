class Solution {
public:
  void all_words(vector<vector<string>> &idx_strings, int curr_idx,
                 int patterns_size, string curr_str, vector<string> &answer) {
    if (curr_idx == patterns_size) {
      answer.push_back(curr_str);
      return;
    }
    for (string str : idx_strings[curr_idx]) {
      all_words(idx_strings, curr_idx + 1, patterns_size, curr_str + str,
                answer);
    }
  }
