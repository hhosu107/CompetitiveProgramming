...
public:
  int numSubmatrixSumTarget(vector<vector<int>> &matrix, int target) {
    ...
    // 2) For each pair (x1, y1), (x2, y2), compute partial sum by (x1, y1) +
    // (x2, y2) - (x1, y2) - (x2, y1). -> n^4?
    int m = matrix.size(), n = matrix[0].size();
    vector<vector<int>> partial_row_sum(m + 1, vector<int>(n + 1));
    vector<vector<int>> partial_sum(m + 1, vector<int>(n + 1));
    for (int i = 1; i <= m; i++) {
      for (int j = 1; j <= n; j++) {
        partial_row_sum[i][j] = partial_row_sum[i][j - 1] +=
            matrix[i - 1][j - 1];
    ...
    unordered_map<int, int>
        hashmap; // key: possible partial_sum [r1, 0] to [r2, col]. value:
                 // number of occurrences. For each col, want to find curr_sum -
                 // target from this map, since then that means there are some
                 // partial matrices whose column = col_s is smaller than col,
                 // that [r1, col_s + 1] to [r2, col] has the value target.
    ...
    for (int r1 = 1; r1 <= m; r1++) {
      for (int r2 = r1; r2 <= m; r2++) {
        ...
        for (int col = 1; col <= n; col++) {
          ...
          count +=
              hashmap[curr_sum -
                      target]; // since curr_sum has not yet been added to the
                               // hashmap, this only finds 'residual matrices'
                               // that satisfies the description of line 17.
          hashmap[curr_sum] += 1;
        }
      }
    }
    ...
    for(int i=1; i<=m; i++) {
        for(int j=1; j<=n; j++) {
            for(int k=i; k<=m; k++) {
                for(int l=j; l<=n; l++) {
                    int subsum = partial_sum[k][l] - partial_sum[k][j-1] -
    partial_sum[i-1][l] + partial_sum[i-1][j-1]; if (target == subsum) count +=
    1;
                }
            }
        }
