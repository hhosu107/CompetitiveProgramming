...
public:
  ...
  const int magic_plate =
      (1 << 0) + 2 * (1 << 3) + 3 * (1 << 6) + 4 * (1 << 9) + 5 * (1 << 12);
  ...
  int bfs(int game_plate_num, int zero_idx) {
    ...
    while (!game_idx.empty()) {
      ...
      for (int next_zero_idx : next_zero_mvmts[curr_zero_idx]) {
        int exchangable_cellval =
            return_cellval(next_zero_idx, curr_game_plate_num);
        ...
        if (game_plate[next_game_plate_num] != -1)
          continue;
  ...
  int slidingPuzzle(vector<vector<int>> &board) {
    ...
    // Our goal is finding whether or not [001010011100101000(little_endian)] is
    // reachable.
    ...
    if (magic_plate == init_game_plate_num)
      return 0;
