...
public:
  ...
  int maximalRectangle(vector<vector<char>> &matrix) {
    ...
    int max_area = 0;
    vector<int> dp(
        matrix[0].size()); // per each row, update the number of consecutive 1s
                           // before this cell in the each column.
    for (int i = 0; i < matrix.size(); i++) {
      ...
      max_area =
          max(max_area,
              hist_maxarea(dp)); // Note that, finiding maximum rectangular in
                                 // the histogram needs O(m) time complexity.
    }
    return max_area;
  }
};
