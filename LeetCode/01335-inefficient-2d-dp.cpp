...
public:
  int minDifficulty(vector<int> &jobDifficulty, int d) {
    ...
    if (len < d)
      return -1;
    // Sol) DP once more?
    // 1) Construct cost[i][j]: for i<=j, this is the cost for finishing jobs
    // between i to j. It needs n^2.
    ...
    // 2) conduct dp[x][i] = min(cost[i][j] + dp[x+1][j+1]) for i in x to len -
    // (d - x), j in i to len - (d - x) inclusive.
