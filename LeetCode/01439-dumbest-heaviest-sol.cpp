...
public:
    ...
    bool increase_idx(int r, vector<vector<int>>& mat) {
        if (mat[0].size() == indices[r] + 1) return false;
        sum -= mat[r][indices[r]];
        indices[r]++;
        sum += mat[r][indices[r]];
        return true;
    ...
    bool operator<(const SelectIdx &o) const {
        return sum < o.sum;
    }
    bool operator>(const SelectIdx &o) const {
        return sum > o.sum;
...
public:
    ...
    // thus, there will be at most k * m different element in the pq when we finish the job.
