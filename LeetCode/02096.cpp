...
// otherwise return -1
    ...
    /* Compare the paths to get the first different value */
        ...
        for (i = 0; i < path1.size() && i < path2.size() ; i++)
            if (path1[i] != path2[i])
                break;
        // starting from i-1, convert path1's remaining characters to U and insert directions for path2.
        string directions = "";
    ...
    // Sol) Find LCA. Save root -> ... -> LCA -> leftNode, root-> ... -> LCA -> ... -> rightNode.
    // Then write "U" [leftNode - LCA] times, write either "L" or "R" by comparing path[j] with either path[j-1]->left or path[j-1]->right.
