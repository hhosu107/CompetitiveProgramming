...
public:
  int assignBikes(vector<vector<int>> &workers, vector<vector<int>> &bikes) {
    auto dist = [&](const int &i, const int &j) {
      return abs(workers[i][0] - bikes[j][0]) +
    ...
    auto costComp = [](const costBike &a, const costBike &b) {
      ...
      if (a.bike_bitset != b.bike_bitset)
        return a.bike_bitset > b.bike_bitset;
      return false;
    };
    ...
    while (true) {
      ...
      costBike c = pq.top();
      pq.pop();
      if (user_bike_seen.find({c.num_users, c.bike_bitset}) !=
          user_bike_seen.end())
        continue;
      ...
      for (int j = 0; j < bikes.size(); j++) {
        if ((c.bike_bitset & (1 << j)) == 0) {
          pq.push({c.cost + dist(c.num_users, j), c.num_users + 1,
                   c.bike_bitset | (1 << j)});
        }
      }
    }
