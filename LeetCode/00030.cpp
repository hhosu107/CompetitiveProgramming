...
public:
    void slidingWindow(int left, string s, vector<int> &answer) {
        ...
        bool wordExceeded = false; // If a word in the substring occurs more than expected, turn it as true.
        ...
        for (int right = left; right <= n - wordLength; right += wordLength) {
            ...
            if (wordCount.find(sub) == wordCount.end()) {
                ...
                while (right - left == substringSize || wordExceeded) {
                    ...
                    if (wordsFound[leftMostWord] >= wordCount[leftMostWord]) { // Even after decreasing, wordsFound is larger than or equal to wordCount.
                        wordExceeded = false;
                    } else {
                        wordsUsed -= 1;
                    }
    ...
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        // With sliding window, we try to catch every substring that starts at wordLength * k + rem.
        // Thus loop on 0<=rem<=wordLeng-1.
        vector<int> answer(0);
