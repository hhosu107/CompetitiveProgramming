...
public:
    bool find132pattern(vector<int>& nums) {
        // Sol) Find strict increasing sequence from left to right; right to left; and find any possibility.
        // What we need to track is: from the right side, which is the largest one that is strictly smaller than current element?
        ...
        for(int i=1; i<len; i++) {
            ...
            */
            if (nums[i] < nums[left_min_idx[i-1]]) {
                left_min_idx[i] = i;
            } else left_min_idx[i] = left_min_idx[i-1];
            rightside_elements.insert(nums[len - 1 - i]);
            it = rightside_elements.lower_bound(nums[len - 1 - i]); // this finds nums[len - 1 - i]; if it is the begin, nums[len - 1 - i] is the smallest; otherwise, --it is the element we wanted to find.
            if (it == rightside_elements.begin()) continue;
            --it;
        ...
        for(int i=1; i<len-1; i++) {
            if (nums[i] > nums[left_min_idx[i]] && nums[i] > right_second_max_elmt[i] && right_second_max_elmt[i] > nums[left_min_idx[i]]) {
                return true;
            }
        }
        return false;
