  ...
  bool areSentencesSimilar(vector<string> &sentence1, vector<string> &sentence2,
    ...
    for (int i = 0; i < sentence1.size(); i++) {
      ...
      if (similarPairMap.find(sentence1[i]) != similarPairMap.end()) {
        if (similarPairMap[sentence1[i]].find(sentence2[i]) !=
            similarPairMap[sentence1[i]].end())
          continue;
      }
      if (similarPairMap.find(sentence2[i]) != similarPairMap.end()) {
        if (similarPairMap[sentence2[i]].find(sentence1[i]) !=
            similarPairMap[sentence2[i]].end())
          continue;
      }
      return false;
    }
