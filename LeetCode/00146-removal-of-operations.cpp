struct DoublyNode {
    ...
    DoublyNode* prev;
    DoublyNode* next;
    DoublyNode(int key, int val) : key(key), val(val), prev(nullptr), next(nullptr) {}
    DoublyNode(int key, int val, DoublyNode* prev, DoublyNode* next) : key(key), val(val), prev(prev), next(next) {}
};
